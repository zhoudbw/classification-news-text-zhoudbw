...
file_classes = open("model_res3\classes.txt", "r", encoding="utf-8")
...
def get_features(test_single_example):
    ...
    tokenizer = BertTokenizer.from_pretrained('model_res3')
...
def main(line):
    ...
    for batch in test_single_example_dataloader:
        ...
        with torch.no_grad():
            ...
            print("新闻类型：", possible)
            possible_list3.append(possible)
            time_end2 = time.time()
            model_time = time_end - time_start  # 模型加载时间
            run_time = time_end2 - time_end  # 模型运行时间
    return model_time, run_time, possible_list3


# 在测试集合上预测
if __name__ == '__main__':
    line = '宝贝梦网国战在即 号召各国招兵买马'
    model_time, run_time, possible_list3 = main(line)
    print('类别：', possible_list3)
